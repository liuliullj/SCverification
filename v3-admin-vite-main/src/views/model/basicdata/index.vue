...
import { getProjectNameApi,getBasicDataDataApi, CreatBasicDataDataApi, updateBasicDataDataApi, deleteBasicDataDataApi} from "@/api/basicdata/index"
import { type GetBasicDataRequestData ,type CreateOrUpdateBasicDataRequestData} from "@/api/basicdata/types/table"
...
const handleCreateOrUpdateBasicData = () => {
  formRef.value?.validate((valid: boolean, fields) => {
    ...
    loading.value = true
    const api = formData.value.id === undefined ? CreatBasicDataDataApi : updateBasicDataDataApi
...
const handleDelete = (row) => {
  ElMessageBox.confirm(`正在删除基础数据类型：${row.basicDataName}，确认删除？`, "提示", {
    confirmButtonText: "确定",
    cancelButtonText: "取消",
    type: "warning"
  }).then(() => {
    deleteBasicDataDataApi(row.id,selectedProject.value).then(() => {
...
//region 改
const handleUpdate = (row: GetBasicDataRequestData) => {
...
const fetchBasicData = () =>{
  ...
  .then((data) => {
    ...
    basicDataData.value = basicDatalist
    })
}

/** 监听分页参数的变化 */
watch([() => paginationData.currentPage, () => paginationData.pageSize], fetchBasicData, { immediate: true })
...
<template>
  <div class="app-container">
    <el-card v-loading="loading" shadow="never" class="search-wrapper">
      ...
      <el-select v-model="selectedProject" placeholder="请选择一个项目" size=“large” style="width: 240px" @change="fetchBasicData" @click="fetchProjectNames">
        <el-option
      ...
      <div class="toolbar-wrapper">
        ...
        <div>
          <el-tooltip content="刷新当前页">
            <el-button type="primary" :icon="RefreshRight" circle @click="fetchBasicData" />
          </el-tooltip>
        </div>
      </div>
      <div class="table-wrapper">
        <el-table :data="basicDataData">
    ...
    <el-dialog
      ...
      :title="formData.id === undefined ? '新增基础数据类型' : '修改基础数据类型'"
      @closed="resetForm"
      width="30%"
    >
      ...
      <template #footer>
        ...
        <el-button type="primary" @click="handleCreateOrUpdateBasicData" :loading="loading">确认</el-button>
      </template>
    </el-dialog>
  </div>
</template>
